...
class TspLibParserTest {
    ...
    fun parse_matrix() {
        ...
        val expected = arrayOf(intArrayOf(0, 2, 5), intArrayOf(2, 0, 2), intArrayOf(5, 2, 0))
    ...
    fun parse_scaled_matrix() {
        val data = """NAME : att4            
            ...
            3 4 4
            EOF"""
        val parser = TspLibParser(data, true)
        val actual = parser.getMatrix()
        val expected =
            arrayOf(intArrayOf(0, 282, 565), intArrayOf(282, 0, 282), intArrayOf(565, 282, 0))
        assertNotNull(actual)
        for (row in actual!!.indices) {
            for (col in 0 until parser.size) {
                assert(expected[row][col] == actual[row][col])
